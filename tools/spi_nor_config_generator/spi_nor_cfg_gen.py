...
from override_cfg import apply_output_overrides, apply_source_overrides, override_chip_cfg
...
Check vendor ID for {name}""".format(vid=vendor_id, name=entry["chip_name"]))
    ...
    # Apply imported-capability corrections, generate the inferred opcode list, and finally apply
    # explicit module attributes. Keeping the second stage after inference allows local opcode
    # corrections to survive changes in the upstream databases.
    for entry in uniqs:
        override = override_chip_cfg.get(entry["chip_name"], {})
...
# Per-device corrections belong in 'override_cfg.py'; do not edit this file directly.
...
""")
        ...
        for entry in uniqs:
            pyfile.write(generate_class(entry["vendor_id"],
                                        ...
                                        entry.get("dummy_cycles"),
                                        entry.get("quad_enable"),
                                        )
                        )
